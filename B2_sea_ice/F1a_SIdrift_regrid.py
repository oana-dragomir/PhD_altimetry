...
> Interpolate data from 25x25km to SSH grid (0.5 lat x 1 lon)
...
for k in range(len(yr)):
	...
	print("Interpolate every month on altimetry grid .. \n")
	# interpolate monthly data onto altimetry
	for i in range(timdim):
		itp_u = griddata((lon.flatten(), lat.flatten()), 
		                um[i,:,:].flatten(),
		                (alt_glon, alt_glat), method='linear')
		itp_v = griddata((lon.flatten(), lat.flatten()), 
		                vm[i,:,:].flatten(),
		                (alt_glon, alt_glat), method='linear')
		"""
		...
		"""
		if i==0:
			u_grid, v_grid = itp_u, itp_v
		else:
			u_grid = np.dstack((u_grid, itp_u))
			v_grid = np.dstack((v_grid, itp_v))
	...
	newfilename = 'SIvel_monthly_p5x1_' + str(yr[k]) + '.nc'
