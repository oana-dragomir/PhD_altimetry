...
from analysis_functions.aux_func import detrend_Dataset
...
def climate_index(index, date_start, date_end, plots):
	...
	if index == 'asl':
		...
		# remove linear trend
		asl_det = detrend_Dataset(asl_anom, 0.95)
		asl_det_stand = asl_det/asl_det.std(ddof=1)

		# remove linear trend and mean seasonal cycle
		asl_det_mclim = (asl_det.groupby('time.month')
			- asl_det.groupby('time.month').mean('time'))
		asl_det_mclim_stand = asl_det_mclim / asl_det_mclim.std(ddof=1)

		# PLOTS
		if plots == 'y':
			x = asl_anom.time
			plot_index(asl_det_mclim_stand.lon, x, 'ASL lon')
			plot_index(asl_det_mclim_stand.lat, x, 'ASL lat')
			plot_index(asl_det_mclim_stand.ActCenPres, x, 'ASL ActCenPres')
			plot_index(asl_det_mclim_stand.SectorPres, x, 'ASL SectorPres')
			plot_index(asl_det_mclim_stand.RelCenPres, x, 'ASL RelCenPres')

		return asl_anom, asl_stand, asl_det, asl_det_stand, asl_det_mclim, asl_det_mclim_stand
	#----------------------------------------------
	...
	#----------------------------------------------
		...
		a_anom = a_crop - a_crop.mean('time')
		a_stand = a_anom / a_anom.std(ddof=1)

		# remove linear trend
		a_det = detrend_Dataset(a_anom, 0.95)
		a_det_stand = a_det / a_det.std(ddof=1)
		#a_mclim_stand = (a_stand.groupby('time.month') 
		#					- a_stand.groupby('time.month').mean('time'))
		a_det_mclim = a_det.groupby('time.month') - a_det.groupby('time.month').mean("time")
		a_det_mclim_stand = a_det_mclim / a_det_mclim.std(ddof=1)

		# PLOTS
		if plots == 'y':
			xtim = a_anom.time.values
			plot_index(a_det_mclim_stand["Nino3.4r"], xtim, 'Nino 3.4')

		return a_anom, a_stand, a_det, a_det_stand, a_det_mclim, a_det_mclim_stand 
	#----------------------------------------------
	...
	#----------------------------------------------
		...
		# standardise data (remove mean, divide by std; mean=0, std=1)
		indx_anom = indx - indx.mean('time')
		indx_stand = indx_anom / indx.std(ddof=1)

		#remove linear trend
		indx_det = detrend_Dataset(indx_anom, 0.95)
		indx_det_stand = indx_det/indx_det.std(ddof=1)
		#indx_mclim_stand = (indx_stand.groupby('time.month') 
		#									- indx_stand.groupby('time.month').mean('time'))
		indx_det_mclim = (indx_det.groupby('time.month') 
						- indx_det.groupby('time.month').mean('time'))
		indx_det_mclim_stand = indx_det_mclim / indx_det_mclim.std(ddof=1)


		# PLOTS
		if plots == 'y':
			xtim = indx.time.values
			plot_index(indx_det_mclim_stand.sam, xtim, 'SAM')
			plot_index(indx_det_mclim_stand.soi_ncep*(-1), xtim, 'SOI (NCEP) x (-1)')
			plot_index(indx_det_mclim_stand.soi_ncar*(-1), xtim, 'SOI (NCAR) x (-1)')
			plot_index(indx_det_mclim_stand.nino3, xtim, 'Nino 3')
			plot_index(indx_det_mclim_stand.nino34, xtim, 'Nino 3.4')

		return indx_anom, indx_stand, indx_det, indx_det_stand, indx_det_mclim, indx_det_mclim_stand
